...
| [システム構成](docs/ARCHITECTURE.md) | アーキテクチャ・技術スタック・処理フロー |
| [DB設計](docs/DATABASE.md) | テーブル定義・ステータス値・遷移図 |
| [API仕様](docs/API.md) | エンドポイント一覧・リクエスト・レスポンス |
| [フロー図](docs/FLOW.md) | アクティビティ図・状態遷移図 |
| [画面設計](docs/SCREENS.md) | 画面一覧・構成要素・画面遷移 |
